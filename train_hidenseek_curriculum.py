        ...
        for j in range(num_case):
            ...
            if fixed_ramp:
                ...
                for i in range(now_ramp_num):
                    poses = [np.array([np.random.randint(start_boundary_quadrant[0][0], start_boundary_quadrant[0][1]),start_boundary_quadrant[0][2]]),
                            np.array([start_boundary_quadrant[1][0], np.random.randint(start_boundary_quadrant[1][1], start_boundary_quadrant[1][2])])]
            ...
            while add_num < self.reproduction_num:
                for k in range(len_start):
                    ...
                    for i in range(s_len):
                        delta_x_direction = random.sample([-self.max_step,self.max_step],1)[0]
                        delta_y_direction = random.sample([-self.max_step,self.max_step],1)[0]
...
def main():
    ...
    start_boundary_quadrant = [[15,26,16],[12,1,11]] # 分别代表ramp[x_left,x_right,y_start] [x_start,y_left,y_right]
